...
- Session windows: three fixed 30-minute windows each day — morning, after lunch, after dinner — shown as a live countdown right on his own HQ screen, with all three sessions' status visible at a glance (used, remaining, or not started yet). The moment he finishes EVERY morning-period AND lunch-period mission, his Chromebook's recreation sites (his own real list, visible on the Missions tab) unlock automatically for the rest of the day — nobody has to flip a switch for him, and nothing unlocks before that. If a session's 30 minutes run out while he's actively using it, that's a friendly heads-up to log off, not a punishment — nothing gets undone or taken away. Right now two of the boys share one Chromebook and have to BOTH finish morning+lunch before that shared device unlocks — a genuine team effort, not "whoever finishes first covers the other."
...
// Regenerate button targets: mostRecentMonday(easternDateStr()).
//
// Skips Monday entirely (John's fix): on a Monday, mostRecentMonday
// resolves to TODAY, so this would otherwise spend a real Sonnet call
// building a "week of [today]" report with essentially zero real data in
// it (the week just started) -- and nothing ever displays that key on a
// Monday in the first place now (Bridge/Officers' Country switch to
// showing the just-finished week's rollup instead, see renderReportCard).
// autoGenerateWeeklyReportCard below already runs at this exact same hour
// on Mondays and does the day's actual real job -- finalizing the week
// that just ended -- so skipping here isn't losing anything, just not
// paying for a report nobody will ever see.
exports.autoGenerateDailyReportCard = functions
  ...
  .onRun(async () => {
    ...
    if (parseDateStr(todayEastern).getUTCDay() === 1) return null; // Monday -- see comment above
    const weekOf = mostRecentMonday(todayEastern);
